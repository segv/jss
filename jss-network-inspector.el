...
(defun jss-console-switch-to-io-inspector (io)
  ...
  (if (jss-io-buffer io)
    ...
    (with-current-buffer (get-buffer-create (jss-io-buffer-name io))
      ...
      (jss-eol-mark)
      (jss-section-marker)
      (insert "Request Headers:") (jss-insert-button "[view raw]" 'identity) (insert "\n")
      (jss-network-inspector-insert-header-table (jss-io-request-headers io))
      (jss-section-marker)
      (insert "Request Data: ") (jss-insert-button "[view raw]" 'identity) (insert "\n")
      (jss-section-marker)
      (when (jss-io-response-status io)
        (insert "Response:\n")
        (insert (jss-io-response-status io) "\n")
        (jss-section-marker)
        (insert "Response Headers: ") (jss-insert-button "[view raw]" 'identity) (insert "\n")
        
        (jss-network-inspector-insert-header-table (jss-io-response-headers io))
        (jss-section-marker)
        (insert "Response Data: ") (jss-insert-button "[view raw]" 'identity) (insert "\n")
        (jss-section-marker))
